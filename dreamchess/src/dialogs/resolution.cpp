...
gg_dialog_t *dialog_resolution_create(gg_dialog_t *parent)
{
	...
	gg_widget_subscribe_signal_name(widget, widget->id, "action_pressed", dialog_ok_cb, old_config);
	gg_container_append(GG_CONTAINER(vbox), widget);
	widget = gg_action_create_with_label("Cancel", 0.5f, 0.0f);
	gg_widget_subscribe_signal_name(widget, widget->id, "action_pressed", dialog_cancel_cb, old_config);
	gg_container_append(GG_CONTAINER(vbox), widget);
