...
void gg_frame_render(gg_widget_t *widget, int x, int y, int focus)
{
	...
	gg_system_draw_rect(x + GG_FRAME_SPACING, y + GG_FRAME_SPACING, width, height, &col_black);

	child->render(child, x + GG_FRAME_SPACING * 2 + GG_FRAME_SIZE, y - GG_FRAME_SPACING * 2 - GG_FRAME_SIZE, focus);
}
...
{
	gg_widget_t *child = gg_bin_get_child(GG_BIN(widget));
	return child->set_focus_pos(child, x - 2 * GG_FRAME_SPACING - GG_FRAME_SIZE,
...
{
	gg_frame_t *frame = (gg_frame_t *)malloc(sizeof(gg_frame_t));
