...
gg_dialog_t *dialog_title_newgame_create(gg_dialog_t *parent)
{
    ...
    gg_widget_subscribe_signal_name(widget, widget->id, "option_changed", dialog_title_cb, (void *)"game_mode");
    vbox2 = gg_vbox_create(0);
    ...
    widget = gg_option_create();
    create_option_values(GG_OPTION(widget), option);
    gg_widget_subscribe_signal_name(widget, widget->id, "option_changed", dialog_title_cb, (void *)"difficulty");
    gg_container_append(GG_CONTAINER(vbox2), widget);

    option = config_get_option("level");
    widget = gg_option_create();
    create_option_values(GG_OPTION(widget), option);
    gg_widget_subscribe_signal_name(widget, widget->id, "option_changed", dialog_title_cb, (void *)"level");
    gg_container_append(GG_CONTAINER(vbox2), widget);
