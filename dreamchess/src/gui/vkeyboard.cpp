...
void populate_key_table(void) {
	...
	for (i = 0; i < 94; i++) {
		keys[i] = (ui_event_t)(i + 33);
	}
}

void toggle_vkeyboard_enabled(void) { vkeyboard_enabled = 1 - vkeyboard_enabled; }

int get_vkeyboard_enabled(void) { return vkeyboard_enabled; }
	...
	for (j = 0; j < 6; j++) {
		...
		for (i = 0; i < 16; i++) {
			if (k < 127 - 33) {
				...
				gg_widget_subscribe_signal_name(action, action->id, "action_pressed", dialog_vkeyboard_key, &keys[k]);
