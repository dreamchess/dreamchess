...
static void draw_health_bar(coord3_t position, coord3_t size, int white) {
	...
	if (white) {
		...
		draw_rect_fill((int)(position.x + (size.x - bar_len)), (int)position.y, (int)bar_len, (int)size.y,
					   get_col(COL_WHITE));
		draw_rect_fill_gradient((int)(position.x + 1 + (size.x - bar_len)), (int)position.y + 1, (int)bar_len - 2,
								(int)size.y - 2, &bottom_right_col, &bottom_left_col, &top_right_col, &top_left_col);
	}
}
...
void draw_ui_elements(void) {
	...
	int clocks = FALSE;
	const char *white_clock, *black_clock;
	...
	white_name_size.x = (float)namew;
	white_name_size.y = (float)nameh;
	black_name = get_black_name();
	gg_system_get_string_size(black_name, &namew, &nameh);
	black_name_size.x = (float)namew;
	black_name_size.y = (float)nameh;

	/* Get clock sizes, string */
	white_clock = "00:00";
	gg_system_get_string_size(white_clock, &namew, &nameh);
	white_clock_size.x = (float)namew;
	white_clock_size.y = (float)nameh;

	black_clock = "00:00";
	gg_system_get_string_size(black_clock, &namew, &nameh);
	black_clock_size.x = (float)namew;
	black_clock_size.y = (float)nameh;
...
void draw_capture_list(coord3_t offset, gg_colour_t *col) {
	...
	for (i = 9; i > 0; i -= 2) {
		...
		draw_texture(get_white_piece((i - 1) / 2), 640 - offset.x, offset.y, 24, 24, 1.0f, get_col(COL_WHITE));
		/* }*/
		offset.y -= 28; /*get_text_character('a')->height;*/
	}
}
