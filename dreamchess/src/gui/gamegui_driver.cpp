...
gg_event_t convert_event(SDL_Event *event) {
	...
	case SDL_MOUSEBUTTONUP: {
		...
		mouse_x = event->motion.x * g_Backend->get_screen_width() / width;
		mouse_y = event->motion.y * g_Backend->get_screen_height() / height;
		...
		gg_event.mouse.x = (int)(((float)mouse_x / (float)g_Backend->get_screen_width()) * 640);
		gg_event.mouse.y = SCREEN_HEIGHT - 1 - (int)(((float)mouse_y / (float)g_Backend->get_screen_height()) * 480);
	} break;
