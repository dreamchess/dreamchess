...
{
    parent_class = (gg_class_id *)realloc(parent_class, (classes + 1) * sizeof(gg_class_id));
...
gg_widget_t *gg_check_cast(gg_widget_t *widget, gg_class_id id, const char *file, int line, const char *type)
{
    if (!widget)
    {
...
void gg_system_draw_string(const char *s, int x, int y, gg_colour_t *colour, int bounce, float align)
{
    int i;
