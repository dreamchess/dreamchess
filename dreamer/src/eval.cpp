...
static int eval_king_tropism(const Board &board, int side) {
	int score = 0;
	int square;
...
static int eval_rook_bonus(const Board &board, eval_data_t *eval_data, int side) {
	int score = 0;
	int square;
...
static int eval_development(const Board &board, int side) {
	int score = 0;
	int square;
	bitboard_t bitboard;
...
static int eval_bad_bishops(const Board &board, eval_data_t *eval_data, int side) {
	int square;
	int score = 0;
...
static int eval_pawn_structure(const Board &board, eval_data_t *eval_data, int side) {
	int score = 0;
	int bin;
...
static void analyze_pawn_structure(const Board &board, eval_data_t *eval_data, int side) {
	int i;
	for (i = 0; i < 8; i++) {
...
static int board_eval_material(const Board &board, int side) {
	int mat_total;
	...
	if (board.material_value[SIDE_BLACK] > board.material_value[SIDE_WHITE]) {
		...
		int val = (2400 < mat_diff ? 2400 : mat_diff) + (mat_diff * (12000 - mat_total) * board.num_pawns[SIDE_BLACK]) /
		...
		int val = (2400 < mat_diff ? 2400 : mat_diff) + (mat_diff * (12000 - mat_total) * board.num_pawns[SIDE_WHITE]) /
